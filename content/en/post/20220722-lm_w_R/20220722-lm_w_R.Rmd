...
Models are a simplification of reality to specific scenarios, so when we intend to apply any model -such as the linear model presented here- to our data, we must check if the assumptions of the model are met. Otherwise our model, its metrics and predictions may mislead the analysis. The assumptions necessary for the model are as follows:<br><br> 

- **Suitability**
   
    The same model can be used for all the observations.
    
- **Linearity**
   
    There is a true linear relationship between the $Response$ and each quantitative explanatory variable of the model.
...
- **Independence**
    ...
    The $Response$ values are independent of each other.<br><br>
